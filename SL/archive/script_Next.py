...
next_games['Punkte Heim'] = np.where(next_games['Heimgewinn'] > next_games['Auswärtsgewinn'], 3, np.where(next_games['Heimgewinn'] == next_games['Auswärtsgewinn'], 1, 0))

# Create a new column 'Punkte Auswärts' and assign points based on predictions
next_games['Punkte Auswärts'] = np.where(next_games['Auswärtsgewinn'] > next_games['Heimgewinn'], 3, np.where(next_games['Heimgewinn'] == next_games['Auswärtsgewinn'], 1, 0))

# Drop the prediction columns and save the results to a CSV file
next_games.drop(['Heimgewinn', 'Auswärtsgewinn', 'Unentschieden', 'Wo', 'Tag', 'Uhrzeit', 'Ergebnis', 'Zuschauerzahl', 'Spielort', 'Schiedsrichter', 'Spielbericht', 'Hinweise'], axis=1, inplace=True)
